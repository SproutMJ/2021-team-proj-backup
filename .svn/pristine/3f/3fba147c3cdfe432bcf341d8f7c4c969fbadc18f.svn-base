...
//					System.out.println(t);
	...
	public static List<Tuple> lz77Encode(List<Byte> origin, int searchBufSize, int lookaheadBufSize) {
		...
		List<Byte> lookaheadBuffer = new ArrayList<>(lookaheadBufSize);
		int size = 0;
		int num = 0;
		...
		while (true) {
			...
			// 2단계
//			long index = searchBuffer.indexOf(lookaheadBuffer.get(0));
//			//찾는 부분이 없을 경우
//			if (index == -1 || lookaheadBuffer.size() == 1) {
//				tuples.add(new Tuple(lookaheadBuffer.get(0)));
//			} 
//			//하나라도 찾으면 최장 공통 부분을 찾는다
//			else {
//				int max_length = 1;
//				int distance = 0;
//				
...
//				tuples.add(new CompressedTuple(tupleIndex, (byte) length, lookaheadBuffer.get(0)));
//			}
			int length = 0;
